module.exports = angular.module("angularjsSeed").factory("noteCrudService", ["$q", "NgTableParams", "$http", "url", function ($q, NgTableParams, $http, url) {
    ...
    return {
        ...
        fetchColumns: [{ field: "title", visible: true }, { field: "body", visible: true }],
        ...
        getEntity: function () {
            return {
                ...
                fields:
                [
                    { type: "text", name: "title", label: "Titulo", required: true, data: "" },
                    { type: "text", name: "body", label: "Cuerpo", required: true, data: "" }/*,
        ...
        fetchAll: function () {
            ...
            $http.get(url + "apiv1/notes").then(function (response) {
                defered.resolve(response.data.notes);
            });
            return promise;
        },
        ...
        edit: function (item) {
            ...
            $http.get(url + "apiv1/notes/" + item.id).then(
                function (response) {
                    defered.resolve(response.data.note);
                });
            return promise;
        },
